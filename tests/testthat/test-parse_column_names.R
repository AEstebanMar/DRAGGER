test_that("exact matches work", {
  test_df <- data.frame(p_value = integer(0), beta_value = integer(0),
                ...
                drug_name = integer(0))
  parsed_colnames <- colnames(parse_column_names(test_df, "GWAS"))
  expected_colnames <- c("p_val_variant", "beta_value", "rs_id", "gene_symbol",
                          "drug_name")
...
test_that("multiple matches to p_value column return error", {
  ...
  expect_error(suppressWarnings(parse_column_names(test_df, "GWAS")), "exceed max allowed value")
  expect_error(suppressWarnings(parse_column_names(test_df, "GTEx")), "exceed max allowed value")
})
...
test_that("if multiple matches exist, throw warning and choose first", {
  ...
  parsed_colnames <- colnames(suppressWarnings(parse_column_names(test_df, "Test")))
  expected_colnames <- c("gene_symbol", "gene_name_bad")
  expect_warning(parse_column_names(test_df, "Test"), "Multiple matches")
  expect_equal(parsed_colnames, expected_colnames)
})

test_that("columns with no matches are left unchanged", {
    test_df <- data.frame(foo = integer(0), bar = integer(0))
    expect_equal(parse_column_names(test_df, "Test"), test_df)
  })
...
test_that("contents of data frame are never changed", {
    test_df <- data.frame(p_value = 1, foo = "bar", gene_name = "TST1",
                          ...
                          NotApplicable = NA)
    parsed_df <- suppressWarnings(parse_column_names(test_df, "GWAS"))
